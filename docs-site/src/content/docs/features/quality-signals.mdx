...
**Loop Detection** watches for repetition. It compares the last four user messages and the last five tool results for similarity, using a confidence threshold of 0.6. When confidence is met, a short warning enters the context. Post-hoc analysis of real sessions found that loop sessions average 47,000 wasted tokens before the user notices. The notes use fixed templates and never echo your content back.
...
| Loop Detection | `UserPromptSubmit` (quality-cache) | Similarity ≥ 0.6 across last 4 user messages or last 5 tool results |
...
| Loop Detection | Similarity threshold | 0.6 confidence |
