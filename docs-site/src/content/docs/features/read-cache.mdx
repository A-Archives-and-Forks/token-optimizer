...
Both of those act on *re-reads*. A related feature, the **first-read skeleton**, serves a skeleton on the *first* read of a large file that history shows is unlikely to be edited soon, with the full original archived and retrievable via `expand`. It is gated on real edit-rate proof per file cohort and is documented with its promotion and tripwire rules on the [Benchmarks page](/reference/benchmarks/#first-read-skeleton-promotion).
