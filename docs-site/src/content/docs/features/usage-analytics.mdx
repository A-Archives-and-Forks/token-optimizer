...
Accepted tiers: `anthropic` (default), `vertex-global`, `vertex-regional`, and `bedrock`. The choice persists in `config.json`. Subscription is not a settable tier: it is a billing mode Token Optimizer detects on its own. On a subscription the meaningful unit is not dollars per token, it is **how much work fits inside the plan you already paid for**. A flat fee buys a rate-limit budget; every token Token Optimizer keeps out of context, and every turn it routes to a lighter model, is budget you get to spend on real work instead. That is why subscription reporting leads with **runway**, how much longer your 5-hour and 7-day windows last, rather than a per-token price. Runway converts to money the moment a window runs out, because finishing the work then means paying for overage or API calls on top of the subscription. See [Keep-Warm quota value](/features/keep-warm/#the-subscription-quota-value-story).
