...
description: "The waste detectors find idle burns, runaway sessions, and config bloat. Twelve run on Claude Code and Codex; OpenClaw runs its own native set of sixteen, tuned to its scheduled-agent surface."
---
...
Detectors are conservative on purpose. A finding is suppressed below a 0.3 confidence threshold and flagged only when it affects a meaningful share of recent sessions, so the report shows real waste, not noise. Two detectors of the same family never double-count: if a model-routing finding already covers a session, the overlapping check stands down.
...
|---|---|
| Respond-to-bash | `respondToBashCommands` left unset in `settings.json`, so Claude Code spends output tokens on an unrequested reply after every slash-command and `!bash` output. |
...
Ten of the twelve run in the report batch (session history or config). WebSearch routing is a post-hoc nudge over trend data, and PDF ingestion runs inline at the read-cache hook. All twelve surface through the same coach and audit reports.
</Aside>
...
|---|---|---|
| Confidence floor | 0.3 | Findings below this are suppressed. |
...
The twelve session detectors run on Claude Code and Codex and on any runtime where Token Optimizer collects sessions. The OpenClaw security checks run only when Fleet Auditor detects OpenClaw. See the [capability matrix](/reference/capability-matrix/).
