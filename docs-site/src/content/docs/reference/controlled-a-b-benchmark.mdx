...
description: "A controlled, per-mechanism A/B test of Token Optimizer's automatic compression: the same task run with the plugin on vs off, measuring the real billed tokens. Distinct from the production Benchmarks page, which measures real-world usage."
---

This is a **controlled experiment**, not a real-world usage report. For field numbers from a
real user's 30 days, see [Benchmarks](/reference/benchmarks) — the two are different lenses, not
competing figures. This page answers a narrower, causal question: run the **same task** with
