...
:::note
This page reports **real-world usage** — one user's actual sessions over 30 days. For a **controlled experiment** that isolates each compression mechanism (the same task run with the plugin on vs off), see the [Controlled A/B Benchmark](/reference/controlled-a-b-benchmark). Two different lenses, not competing numbers.
:::
...
Every row above is input-side: context Token Optimizer removed before it reached the model. One feature works on the other, more expensive side of the bill. The **Lean-Output Nudge** fires when context fills past 25%, regardless of context quality: a short, cache-safe note (injected as `additionalContext`, never rewriting the existing prefix) tells the model to reason deeply but keep its visible output lean. The reduction is **estimated at 10 to 15%, not measured**. The counterfactual (what the model would have written without the nudge) cannot be observed, so this figure is an assumption recorded in the estimated tier and is never summed into the metered totals above. Every nudge does record the session's pre-nudge average output tokens, so a future calibration can compare actual post-nudge output against that baseline.
