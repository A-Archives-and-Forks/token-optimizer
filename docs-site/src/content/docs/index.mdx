...
{/* href uses BASE_URL, not a hardcoded base: rehype-base-links only rewrites
    anchors in MDX prose, never a component's own href, so this has to survive the
    documented BASE = "" custom-domain switch in astro.config.mjs on its own. */}
<LinkCard
  title="How does it compare to other solutions?"
  href={`${import.meta.env.BASE_URL.replace(/\/$/, '')}/reference/comparison/`}
  description="Token Optimizer against every other context tool, feature by feature, across all 26 rows."
/>
