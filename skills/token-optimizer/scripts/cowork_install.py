...
hooks/hooks.json is trimmed to the event set that fires in Cowork
(SessionStart, UserPromptSubmit, PreToolUse, Stop — v4 correction, to be
re-verified live with to-hook-probe) and keepwarm is dropped (its premise
— keep a local CLI warm — does not transfer to Cowork).
"""
...
# The hook events that fire in Cowork when the plugin arrives via the org
# admin console push (source: v4-CORRECTION — Imri's shipped Keryn plugin +
# real reports). Everything else in hooks/hooks.json (PreCompact,
# PostCompact, SessionEnd, StopFailure, CwdChanged, PostToolUse) is dropped
# from the payload until to-hook-probe proves it fires; hooks are additive
# and fail-open, so a dropped event degrades nothing.
