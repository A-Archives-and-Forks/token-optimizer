...
### Keep-Warm: methodology and honesty rules

Keep-Warm converts the watchdog's Claude Code re-write waste into recoverable dollars by pinging the prompt cache before it expires. The feature is opt-in, API-billing-only. Its mechanism is grounded in a live controlled test; its dollar figure is a history-replay **projection** — the shipping policy code replayed over real pause data, not a separate model. A history-replay of an unshipped-on-this-machine policy is still a projection, so the dollar number reads as projected, never realized.

**Mechanism proof (verified live, single controlled session).** Two load-bearing claims were tested with a real `claude` CLI session in a scratch directory, asserting on `--output-format json` usage fields:

- **A resume-style ping is a cache READ, not a re-write.** A 1h-TTL prefix of **32,960** cached tokens was seeded. The ping (`--resume <id> -p --no-session-persistence`) returned `cache_read = 32,960` (≈ the full prefix) with `cache_creation = 281` (a tiny refresh delta, not a full re-write), and the original transcript file was byte-identical afterward (no new session files written). VERIFIED.
- **The read refreshes the entry so a later real resume stays warm.** With the original 1h window = 3600s, a refresh ping at **3317s** kept the entry alive; the real resume at **3909s** (past the original expiry) still landed `cache_read = 32,960` with no re-write spike. VERIFIED. Total experiment spend: **$0.04**.

**Backfill validation (replay through the live policy code).** The shipping `keepwarm_policy_decision` is replayed step-by-step over the watchdog's own per-session pause data using the same transcript parser the watchdog uses — never a separate model. Three modes are scored: **probe-only** (no predictor), **predictor-sustain** (the real resume predictor over ping-time-computable features only), and **oracle-sustain** (hindsight upper bound). A promotion **fence** enables sustain only when replayed `net(predictor) > net(probe-only) > 0`.

On the dogfood machine (30 days, 568 sessions, read-only replay), all three modes collapsed to the **same +$53.50/30d net** projection (modeled saving $177.23 − modeled ping spend $123.73; 96 writes avoided). No dollars were realized on this machine — it runs on subscription auth, where Keep-Warm stays hard-off, so the figure is a history-replay projection through the shipping policy code, not realized savings. The per-pause break-even gate stops every pause at ≤2 pings at the elapsed times where real resumes occur, so sustain cannot beat probe-only here. The fence correctly **refused** to promote sustain; the probe-only floor ships. This is the implementable result — the offline simulation's +$119 oracle figure used optimal-stopping without the live break-even gate and is a hindsight bound the shipping code does not reach by design. Validating the implementable policy (not the upper bound) is the whole reason the backfill step exists.

**Honesty rules (enforced by tests).**

- A saving is booked as **realized** only when a resume lands on an evidence-verified warm prefix (the resumed turn shows `cache_read ≥ 50%` of the prefix and no re-write spike); the counterfactual is priced at the profile's real cache-write rate.
- **Ping spend is always booked** (written before the subprocess fires, so a crash can't hide it) and rendered as a first-class cost line; net is always visible.
- **Opportunity is never in the realized headline.** Classification alone never books a saving, and realized rows are de-duplicated per pause. There is no double-count against the watchdog's `avoidable_behavioral_usd`: a kept-warm resume simply stops producing a re-write event in the same transcript the watchdog scans.
