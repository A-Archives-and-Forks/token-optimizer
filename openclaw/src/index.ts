      ...
      if (!_continuityInjectedSessions.has(event.sessionId)) {
        ...
        try {
          ...
          if (promptText && event.inject) {
            ...
            const candidate = findBestContinuityCheckpoint(promptText, event.sessionId, cwd);
            if (candidate) {
            ...
            if (candidate) {
              api.logger.info(
                `[token-optimizer] Cross-session match found but no inject path available. ` +
              ...
              storePendingContinuityHint(event.sessionId, buildContinuityHint(candidate));
            }
          } else {
